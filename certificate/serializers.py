...
# CertificateSchedule + Certificate Info
class CertInfoCertScheduleSerializer(serializers.ModelSerializer):
    certificates_name = serializers.CharField(read_only=True, source="cert_id.name")
    certificates_cost = serializers.CharField(read_only=True, source="cert_id.cost")
    certificates_cost_sil = serializers.CharField(read_only=True, source="cert_id.cost_sil")
    class Meta:
        model = CertSchedule
        fields = ('cert_id', 'test_type', 'test_round', 'reg_start_date', 'reg_end_date', 'test_start_date', 'test_end_date', 'result_date_1', 'result_date_2', 'certificates_name', 'certificates_cost', 'certificates_cost_sil')
